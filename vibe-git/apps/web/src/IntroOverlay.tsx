...
export function IntroOverlay() {
  const [visible, setVisible] = useState(() => !sessionStorage.getItem(KEY) && !matchMedia("(prefers-reduced-motion: reduce)").matches);
  const [leaving, setLeaving] = useState(false);
  useEffect(() => {
    if (!visible) return;
    const leave = () => { sessionStorage.setItem(KEY, "1"); setLeaving(true); };
  ...
  return <div className={`intro-overlay ${leaving ? "leaving" : ""}`} aria-label="vibe-git 启动画面">
    ...
    <button className="intro-skip" onClick={() => { sessionStorage.setItem(KEY, "1"); setLeaving(true); }}>跳过动画</button>
  </div>;
}

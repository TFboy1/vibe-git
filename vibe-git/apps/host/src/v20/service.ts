...
import type {
  ...
  ProjectModule, PlanImpactPreview, Workstream, InterfaceContract, TaskBrief, DependencyEdge, ContractUpdate
...
const ALIGNMENT_SCHEMA = {
  ...
  required: ["alignmentMarkdown", "tasks", "issues", "contracts"],
  properties: {
      ...
      if (allDone) {
        ...
        for (const taskId of review.affectedTaskIds) {
          ...
          this.repo.putTask({
            ...
            mockEvidence: contractChanged ? null : task.mockEvidence,
            integrationEvidence: contractChanged ? null : task.integrationEvidence,
